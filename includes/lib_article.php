 ...
 * @return  array
 */
function get_assign_cat_article($where,$limit) {
	$sql = 'SELECT * FROM '.$GLOBALS['ecs']->table('article_cat').' '.$where.' order by sort_order '.$limit;
	...
	if ($res) {
		while ($row = $GLOBALS['db']->fetchRow($res)) {
			...
			$tmp['article'] = get_cat_articles($row['cat_id'],1,$GLOBALS['_CFG']['article_number']);
			$arr[] = $tmp;
		}
	}
